...
Open with a reference so context is clear (`Addressed @<author>'s review (fixed in <sha>):`), then one bullet per named issue:

```
Addressed @claude[bot]'s review (fixed in <sha>):
